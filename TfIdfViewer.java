...
public class TfIdfViewer {
  ...
  private static TermWeight[] toTfIdf(IndexReader reader, int docId) throws Exception {
     ...
     for (int i = 0; i < tw.length; i++) {
         double tf  = (double)freqs[i] / fmax;
         double idf = Math.log((double)nDocs / docFreq(reader, terms[i]));
  ...
  private static void normalize(TermWeight[] t) {
      //aqui sumo todas las frequencias del tw y hago la raiz cuadrada con sqrt. 
      //despues Cada frequencia es dividida por ese numero y tengo el vector normalizado
      int n = t.length;
      double sum = 0.0;
      for (int i = 0; i < n; ++i) sum += t[i].getWeight()*t[i].getWeight();
  ...
  private static double cosineSimilarity(TermWeight[] v1, TermWeight[] v2) {
    ...
    while (i < v1Length && j < v2Length)
    {
        ...
        if (text1.compareTo(text2) < 0) ++i;
        else if (text1.compareTo(text2) > 0) ++j;
        else 
        {
            sum += v1[i].getWeight() * v2[j].getWeight();
            ++i;
            ++j;
        }
    }
